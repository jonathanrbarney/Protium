...
class Course(models.Model):
    ...
    department = models.ForeignKey('Department', on_delete=models.CASCADE, related_name='courses',blank=False)
...
class Enrollment(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4)
    registration = models.ForeignKey('Registration', on_delete=models.SET_NULL, related_name="enrollments", null=True)
    section = models.ForeignKey(Section, on_delete=models.SET_NULL, related_name='enrollments', null=True)
    creator = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, related_name='enrollments_created')
    def __str__(self):
...
class Registration(models.Model):
    ...
    course = models.ForeignKey(Course, on_delete=models.SET_NULL, related_name="registrations", null=True)
    number_grade = models.DecimalField(max_digits=7, decimal_places=4, blank=True, null=True)
    creator = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, related_name='registrations_created')
    term = models.ManyToManyField(Term, blank=True, related_name='registrations')
    def __str__(self):
        return f'{self.student.last_name}, {self.student.first_name} ({self.student.usafa_id})'
...
class Completion(models.Model):
    ...
    course = models.ForeignKey(Course, on_delete=models.SET_NULL, related_name="completions", null=True)
    number_grade = models.DecimalField(max_digits=7, decimal_places=4, blank=True, null=True)
    creator = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, related_name='completions_created')
    GRADE_CHOICES = (
        ('A', 'A'),
        ('A-', 'A-'),
        ('B+', 'B+'),
...
class Course_Requirement(models.Model):
    program = models.ForeignKey('Program', on_delete = models.CASCADE, blank=False)
    id = models.UUIDField(primary_key=True, default=uuid.uuid4)
    name = models.CharField(max_length=300)
